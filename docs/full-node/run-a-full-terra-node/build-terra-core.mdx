---
sidebar_position: 2
---

import Admonition from '@theme/Admonition';

# Build Terra Core

Terra Core is the official Golang reference implementation of the Terra node software. Use this guide to install Terra Core and Terrad, the command-line interface and daemon that connects to Terra and enables you to interact with the Terra blockchain.

## Get the Terra Core source code

<Admonition type="caution" icon="☢️" title="Syncing from genesis">

If you are syncing a node from genesis, you will need to use the appropriate core version for the block height of the respective chain you are syncing. Follow the [sync from genesis](./sync.mdx#sync-from-genesis) guide for step-by-step instructions.

//TODO: Update block heights and versions.

| Network Name | Network Type | Block Height Range  | Core Version |
| :----------: | :----------: | :-----------------: | :----------: |
| `columbus-5` |   Mainnet    | `genesis` - 11734000 |             |
|              |              |  11734000 - 12815210 |    v1.1.0   |
|              |              | 12815210 - 13215800  |    v3       |
|              |              | 13215800 - `present` |    v4       |
| :----------: | :----------: | :-----------------: | :----------: |
|  `rebel-2`   |   Testnet    | `genesis` - 838500  | v2.0.0-rc.0  |
|              |              |  838500 - 2777777   | 2.1.0-beta.1 |
|              |              | 2777777 - 4712048   |    v2.2.0    |
|              |              | 4712048 - `present` | v2.3.0-rc.0  |


</Admonition>

1. Use `git` to retrieve [Terra Core](https://github.com/classic-terra/core), and check out the `main` branch, which contains the latest stable release. You can find the latest tag on the [tags page](https://github.com/terra-money/core/tags) or via autocomplete in your terminal: type `git checkout v` and press `<TAB>`.

   ```
   git clone https://github.com/classic-terra/core
   cd core
   git checkout [latest version]
   ```

2. Build Terra Core. This will install the Terrad executable to your [ `GOPATH` ](https://go.dev/doc/gopath_code) environment variable.

   ```bash
   make install
   ```

3. Verify that Terra Core is installed correctly.

   ```bash
   terrad version --long
   ```

   **Example**:

   ```bash
   name: terra
   server_name: terrad
   version: 2.1.2
   commit: 4838a2c5764f093b6bd65ce8fbd060a29e603b74
   build_tags: netgo,ledger
   go: go version go1.18.0 darwin/arm64
   # ...followed by a lot of dependencies
   ```

<Admonition type="tip" icon="💡">

If the `terrad: command not found` error message is returned, confirm that the Go binary path is correctly configured by running the following command:

```bash
export PATH=$PATH:$(go env GOPATH)/bin
```

</Admonition>
