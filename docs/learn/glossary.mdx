...
This glossary provides definitions of terms specific to the Terra Classic Protocol.
...
When a user [delegates](#delegate) or bonds Luna to a [validator](#validator) to receive [staking rewards](#rewards). Validators never have ownership of a delegator's [Lunc](#lunc), even when bonded. Delegating, bonding, and staking generally refer to the same process.
...
When users or delegators add their [Lunc](#lunc) to a [validator's](#validator) stake in exchange for rewards. Delegated Luna is bonded to a validator. Validators never have ownership of a [delegator's](#delegator) Lunc. Delegating, bonding, and staking generally refer to the same process.
...
For more information on fees, visit [Fees on Terra Classic](./fees.mdx).

## Full node

A computer connected to the [Terra Classic mainnet](#terra-mainnet) that is able to validate transactions and interact with the Terra blockchain. All active [validators](#validator) run full nodes.

## Governance

Governance is the democratic process that allows users and [validators](#validator) to make changes to the Terra Classic protocol. Community members submit, vote, and implement proposals. One staked [Lunc](#lunc) is equal to one vote.

## Governance proposal

A written submission for a change or addition to the Terra Classic blockchain. Topics of proposals can vary from community pool spending, software changes, or parameter changes.
...
The native staking token of the Terra Classic protocol. Lunc is also used as a governance token. [Delegators](#delegator) can stake Luna Classic to receive rewards.
...
A section of the Terra Classic Core that represents a particular function of the Terra Classic protocol. Visit the [Terra Classic Core module specifications](../develop/module-specifications/README.mdx) for more information.

## Columbus-5

The latest version of the [Terra Classic mainnet](#terra-mainnet).
...
A blockchain consensus mechanism where validators are chosen to confirm blocks at a rate proportional to their [stake](#staking). The [Tendermint Concensus](#tendermint-consensus) outlines the proof-of-stake procedure used by Terra Classic.
...
When a delegator wants to transfer their bonded luna to a different validator. Redelegating Luna Classic is instant and does not require a 21-day unbonding period.
...
The amount of Luna Classic a validator bonds to themselves. Also referred to as self-bonding.

## Slashing

Punishment for validators that misbehave. Validators lose part of their stake when they get slashed.

For more information, see [slashing](./protocol.mdx#slashing) in the description of the Terra Classic protocol.
...
The amount of [Lunc](#lunc) bonded to a validator.

## Staking

When a user delegates or bonds their Lunc to an active validator to receive rewards. Bonded Lunc adds to a validator's stake. Validators provide their stakes as collateral to participate in the consensus process. Validators with larger stakes are chosen to participate more often. Validators receive staking rewards for their participation. A validator's stake can be slashed if the validator misbehaves. Validators never have ownership of a delegator's Luna Classic, even when staked.

For more information on staking, visit the [concepts page](./protocol.mdx#staking).

## Tendermint consensus

The proof-of-stake consensus procedure used by the Terra Classic protocol. When a validator proposes a new block, the remaining validators then vote on the block in two voting rounds. If a block receives a two-thirds or greater majority of yes votes in both rounds, it gets added to the blockchain. All validators get rewarded with the block's transaction fees, however, the initial proposer of the block will receive a slightly higher reward. For each new block, a validator is selected to propose based on their [weight](#weight). Check out the [Tendermint official documentation](https://docs.tendermint.com/) for more information.

## Terra Classic Core

The official source code for the Terra Classic protocol.

For more information on the Terra Classic Core, see [Terra Core modules](../develop/module-specifications/README.mdx).

## Terra Classic mainnet

The Terra Classic protocol's blockchain network where all transactions take place.

## Station

Terra Classic's native wallet and platform for swaps, governance, and staking. In Station, you can send, receive, and stake Terra Classic tokens. You can also participate in governance and vote on proposals.
...
A copy of the mainnet network used for testing. The testnet does not use real tokens. You can use the testnet to get familiar with carrying out transactions on Terra. The current testnet for Terra is [`rebel-2`](../develop/endpoints.mdx).

## The Terra Classic ecosystem

A quickly expanding network of decentralized applications built on the Terra Classic protocol.
...
The total amount of Luna Classic bonded to a delegator, including self-bonded Luna Classic.

## Unbonded Luna Classic

Luna Classic that can be freely traded and is not staked to a validator.
...
When a delegator decides to undelegate their Luna Classic from a validator. This process takes 21 days. No rewards accrue during this period. This action cannot be stopped once executed.

## Unbonding Luna Classic

[Lunc](#lunc) that is transitioning from bonded to unbonded. Lunc that is unbonding cannot be traded freely. The unbonding process takes 21 days. No rewards accrue during this period. This action cannot be stopped once executed.
